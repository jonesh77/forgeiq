...
export default function CollectValuesForStrain () {
    ...
    const submitWith = async (fd: FormData) => {
        ...
        if (r.ok && typeof r.data === "string") {
            ...
            toast.success(t("pmap.collect.toast_ok"));
        }
        setLoading(false);
    };
            ...
            <form className="w-full lg:flex-1 lg:border-r lg:pr-10" onSubmit={handleSubmit}>
                <div className="flex items-center justify-between gap-x-3 flex-wrap gap-y-3">
                    <div className="flex items-center gap-x-3">
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{t("pmap.collect.title")}</h1>
                        <Info />
                    </div>
                    <ModeToggle mode={mode} setMode={setMode} />
                </div>
                <p className="mt-1 text-sm text-slate-500">{mode === "quick" ? t("pmap.collect.subtitle_quick") : t("pmap.collect.subtitle_adv")}</p>
                ...
                <Button className="w-[300px] mt-3 h-11 bg-slate-900 hover:bg-slate-800 font-medium" type="submit" disabled={loading}>{
                    loading ? (<><AiOutlineLoading className="animate-spin" />{t("pmap.common.loading")}</>) : (<><TbCube3dSphere />{t("pmap.collect.gen_excel")}</>)
                }</Button>
                </div>
            </form>
            <div className="w-full lg:flex-[5]">
                {data && (
                    <div className="max-w-[380px]">
                        <div className='flex items-center gap-x-3 text-slate-800'><GoGear className='text-base' /><h2 className='text-sm font-medium'>{t("pmap.collect.generated_xlsx")}</h2></div>
                        ...
                        <Button className='cursor-pointer w-full bg-slate-800 hover:bg-slate-900 mt-4' onClick={downloadXlsx}><MdOutlineFileDownload />{t("pmap.collect.download_xlsx")}</Button>
                    </div>
                )}
            </div>
        </div>
