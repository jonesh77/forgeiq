...
function ModelTrainer ({ state, setState }) {
  ...
  const submitWith = async (fd: FormData) => {
    ...
    if (r.ok) {
        ...
        toast.success(data.used_sample ? t("cog.tm.toast_sample_ok") : t("cog.tm.toast_ok"));
        recordHistory({
            service: "cogging.train_model",
            title: data.used_sample ? t("cog.tm.history_sample") : t("cog.tm.history"),
  ...
  return (
    <div className='w-full bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 lg:p-8 lg:py-7 flex flex-col lg:flex-row lg:items-stretch gap-6 lg:gap-0 shadow-sm'>
      <form className='w-full lg:flex-1/3 flex flex-col justify-between relative' onSubmit={handleSubmit}>
        <div>
          <div className='inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-[10px] font-semibold uppercase tracking-wider mb-2'>{t("cog.tm.step1")}</div>
          <div className="flex items-center gap-2">
            <h2 className='text-2xl text-slate-900 font-semibold tracking-tight'>{t("cog.tm.title")}</h2>
            <Info />
          </div>
          <h4 className='text-[13px] mt-1 w-4/5 text-slate-500 leading-relaxed'>{t("cog.tm.subtitle")}</h4>
        ...
        <Button type="submit" className='cursor-pointer h-11 mt-6 bg-slate-900 hover:bg-slate-800 font-medium' disabled={state.status === "loading"}>
          <FaGears />{t("cog.tm.generate_model")}
        </Button>
      </form>
        ...
        {state.status == "steady" ? (
          <div className="h-full w-full rounded-2xl bg-slate-50 border border-dashed border-slate-300 flex items-center justify-center">
            <div className='text-center text-slate-500'>
              ...
              <p className='mt-3 text-sm font-medium text-slate-700'>{t("cog.tm.placeholder_title")}</p>
              <p className='text-xs mt-1'>{t("cog.tm.placeholder_sub")} <span className="text-amber-700 font-medium">{t("sample.try")}</span> {t("cog.tm.placeholder_sub_after")}</p>
            </div>
          </div>
          ...
          <Skeleton className="h-full w-full rounded-2xl bg-slate-100 flex items-center justify-center">
            <div className='flex flex-col items-center text-slate-600'>
              ...
              <p className='mt-5 text-sm font-medium text-slate-800'>{t("cog.tm.loading_title")}</p>
              <p className='text-xs mt-2 max-w-[280px] text-center leading-relaxed text-slate-500'>{t("cog.tm.loading_sub")}</p>
            </div>
          </Skeleton>
        ) : state.status == "filled" ? (
          <div className='w-full h-full grid grid-cols-3 gap-x-6 0'>
            <div className='col-span-2 max-h-[400px]'>
              <div className='flex items-center gap-x-3 h-[4%]'>
                ...
                <h2 className='text-sm font-medium'>{t("cog.tm.generated_plot")}</h2>
              </div>
            ...
            <div className='col-span-1 flex flex-col gap-y-4'>
              <div className='flex-grow'>
                <div className='flex items-center gap-x-3 text-slate-800'><GoGear className='text-base' /><h2 className='text-sm font-medium'>{t("cog.tm.generated_h5")}</h2></div>
              ...
              <Button className='cursor-pointer bg-slate-800 hover:bg-slate-900' onClick={downloadH5}><MdOutlineFileDownload />{t("cog.tm.download_h5")}</Button>
            </div>
          </div>
        ) : (
          <></>
...
function TrainDataCorrection ({ state, setState }) {
  ...
  const submitWith = async (fd: FormData) => {
    ...
    if (r.ok) {
        ...
        toast.success(data.used_sample ? t("cog.tdc.toast_sample_ok") : t("cog.tdc.toast_ok"));
        recordHistory({
            service: "cogging.train_correction",
            title: data.used_sample ? t("cog.tdc.history_sample") : t("cog.tdc.history"),
            params: {
      ...
      <form ref={formRef2} className='w-full lg:flex-1/3 flex flex-col justify-between relative' onSubmit={handleSubmit}>
        <div className='pb-6'>
          <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
            <div className='inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-[10px] font-semibold uppercase tracking-wider'>{t("cog.tdc.badge")}</div>
            <ModeToggle mode={mode} setMode={setMode} />
          </div>
          <div className="flex items-center gap-2">
            <h2 className='text-2xl text-slate-900 font-semibold tracking-tight'>{t("cog.tdc.title")}</h2>
            <Info2 />
          </div>
          <h4 className='text-[13px] mt-1 w-4/5 text-slate-500 leading-relaxed'>{mode === "quick" ? t("cog.tdc.subtitle_quick") : t("cog.tdc.subtitle_adv")}</h4>
          <ModeBanner mode={mode} />

          {mode === "advanced" && (
            <>
              <h5 className='mt-5 font-medium text-xs text-slate-700'>{t("cog.tm.excel_file")}</h5>
          ...
          <div className='flex items-center gap-x-2 mt-5'>
            <h5 className='font-medium text-xs text-slate-700'>{t("cog.tdc.target_astm")}</h5>
            <ParamHelp>{t("cog.tdc.target_astm_hint")}</ParamHelp>
          </div>
          <Input type='number' defaultValue="6" name='target_astm' className='bg-slate-50 border-slate-200 mt-1.5 h-10 placeholder:text-sm' />

          <div className='flex items-center gap-x-2 mt-5'>
            <h5 className='font-medium text-xs text-slate-700'>{t("cog.tdc.weight_factor")}</h5>
            <ParamHelp>{t("cog.tdc.weight_factor_hint")}</ParamHelp>
          </div>
        ...
        <Button type="submit" className='cursor-pointer h-11 mt-6 bg-slate-900 hover:bg-slate-800 font-medium' disabled={state.status === "loading"}>{t("cog.tdc.generate")}</Button>
      </form>
      <div className='hidden lg:block w-px bg-slate-200 mx-8'></div>
      <div className='w-full lg:flex-2/3 min-h-[300px] lg:min-h-[400px]'>
        {state.status == "steady" ? (
          <div className="h-full w-full rounded-2xl bg-slate-50 border border-dashed border-slate-300 flex items-center justify-center">
            <div className='flex items-center gap-x-3 text-slate-500 text-sm'><LuFileInput /><p>{t("cog.tdc.placeholder")}</p></div>
          </div>
        ) : state.status == "loading" ? (
          <Skeleton className="h-full w-full rounded-3xl bg-slate-100 flex items-center justify-center">
            <div className='flex flex-col items-center'>
              ...
              <p className='mt-6'>{t("cog.tdc.loading")}</p>
            </div>
          </Skeleton>
        ) : state.status == "filled" ? (
          <div className='w-full h-full grid grid-cols-5 gap-x-6 0'>
            <div className='col-span-2 flex flex-col gap-y-4'>
              <div className='flex-grow'>
                <div className='flex items-center gap-x-3 text-slate-800'><GoGear className='text-base' /><h2 className='text-sm font-medium'>{t("cog.tdc.generated_xlsx")}</h2></div>
              ...
              <Button className='cursor-pointer bg-slate-800 hover:bg-slate-900' onClick={downloadXlsx}><MdOutlineFileDownload />{t("cog.tdc.download_xlsx")}</Button>
            </div>
          </div>
        ) : (
          <></>
