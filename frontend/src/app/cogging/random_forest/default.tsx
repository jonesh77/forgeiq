...
type Metrics = {
  ...
  r2_mean: number;
  pi_width_mean?: number;
  pi_coverage_pct?: number;
};
...
export default function RandomForestForm() {
  ...
  const submitWith = async (fd: FormData) => {
    ...
    const r = await postToBackend1("/api/cogging/random_forest", fd);
    if (r.ok) {
      const data = r.data;
      setState({
        status: "filled",
      ...
      toast.success(data.used_sample ? t("cog.rf.toast_sample_ok") : t("cog.rf.toast_ok"));
      recordHistory({
        service: "cogging.random_forest",
        title: data.used_sample ? t("cog.rf.history_sample") : t("cog.rf.history"),
        params: { n_splits: nSplits, input_file: (fd.get("file") as File)?.name || "sample" },
        summary: `RF · R²=${data.metrics.r2_mean.toFixed(3)} · RMSE=${data.metrics.rmse_mean.toFixed(3)}`,
  ...
  const downloadPkl = () => {
    ...
    link.download = "random_forest_bundle.pkl";
    link.click();
  };
  ...
  const downloadImage = () => {
    ...
    link.download = "rf_diagnostics.png";
    link.click();
  };
  ...
  return (
    <div className="w-full bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 lg:p-8 lg:py-7 flex flex-col lg:flex-row lg:items-stretch gap-6 lg:gap-0 shadow-sm">
      <form className="w-full lg:flex-1/3 flex flex-col justify-between relative" onSubmit={handleSubmit}>
        <div>
          <div className="inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-sky-50 border border-sky-200 text-sky-700 text-[10px] font-semibold uppercase tracking-wider mb-2">
            {t("cog.rf.badge")}
          </div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl text-slate-900 font-semibold tracking-tight">{t("cog.rf.title")}</h2>
          </div>
        ...
        <Button type="submit" className="cursor-pointer h-11 mt-6 bg-sky-700 hover:bg-sky-800 font-medium" disabled={state.status === "loading"}>
          <FaGears />{t("cog.rf.train_button")}
        </Button>
      </form>
        ...
        {state.status === "loading" && (
          <Skeleton className="h-full w-full rounded-2xl bg-slate-100 flex items-center justify-center">
            <div className="flex flex-col items-center text-slate-600">
              <AiOutlineLoading className="animate-spin text-3xl text-sky-700" />
              <p className="mt-5 text-sm font-medium text-slate-800">{t("cog.rf.loading_title")}</p>
          ...
          <div className="w-full h-full grid grid-cols-3 gap-x-6">
            <div className="col-span-2 max-h-[460px]">
              <img
                ...
                alt="Random Forest CV diagnostics"
            ...
            <div className="col-span-1 flex flex-col gap-3">
              <div className="rounded-xl border border-sky-200 bg-sky-50/50 p-4">
                <div className="text-[10px] uppercase tracking-widest text-sky-700 font-semibold">
                  {t("cog.rf.backend_label")}
                </div>
              ...
              <Button onClick={downloadPkl} className="cursor-pointer h-10 bg-slate-900 hover:bg-slate-800">
                <LuDownload />{t("cog.rf.download_pkl")}
              </Button>
