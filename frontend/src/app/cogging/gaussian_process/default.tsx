...
export default function GaussianProcessForm() {
  const { t } = useT();
  ...
  const submitWith = async (fd: FormData) => {
    ...
    const r = await postToBackend1("/api/cogging/gpr", fd);
    if (r.ok) {
      const data = r.data;
      setState({
        status: "filled",
      ...
      toast.success(data.used_sample ? t("cog.gpr.toast_sample_ok") : t("cog.gpr.toast_ok"));
      recordHistory({
        service: "cogging.gaussian_process",
        title: data.used_sample ? t("cog.gpr.history_sample") : t("cog.gpr.history"),
        params: { n_splits: nSplits, input_file: (fd.get("file") as File)?.name || "sample" },
        summary: `GPR · R²=${data.metrics.r2_mean.toFixed(3)} · RMSE=${data.metrics.rmse_mean.toFixed(3)}`,
  ...
  const downloadImage = () => {
    ...
    link.download = "gpr_diagnostics.png";
    link.click();
  };
      ...
      <form className="w-full lg:flex-1/3 flex flex-col justify-between relative" onSubmit={handleSubmit}>
        <div>
          ...
          <div className="flex items-center gap-2">
            <h2 className="text-2xl text-slate-900 font-semibold tracking-tight">{t("cog.gpr.title")}</h2>
          </div>
        ...
        <Button type="submit" className="cursor-pointer h-11 mt-6 bg-violet-700 hover:bg-violet-800 font-medium" disabled={state.status === "loading"}>
          <FaGears />{t("cog.gpr.train_button")}
        </Button>
      </form>
        ...
        {state.status === "loading" && (
          <Skeleton className="h-full w-full rounded-2xl bg-slate-100 flex items-center justify-center">
            <div className="flex flex-col items-center text-slate-600">
              ...
              <p className="mt-5 text-sm font-medium text-slate-800">{t("cog.gpr.loading_title")}</p>
          ...
          <div className="w-full h-full grid grid-cols-3 gap-x-6">
            <div className="col-span-2 max-h-[460px]">
              <img
                ...
                alt="Gaussian Process CV diagnostics"
            ...
            <div className="col-span-1 flex flex-col gap-3">
              ...
              <Button onClick={downloadPkl} className="cursor-pointer h-10 bg-slate-900 hover:bg-slate-800">
                <LuDownload />{t("cog.gpr.download_pkl")}
              </Button>
